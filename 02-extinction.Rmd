...
The success of the Endangered Species Act is often measured by comparing the number of species declared extinct to the number of listed species, which indicates success on the order of 99%. Here we check that measure against the discussions of extinction by the Fish and Wildlife Service and the National Marine Fisheries Service by computationally mining five-year status reviews. We find 79 (~5%) domestic ESA-listed species are known to be or are likely to be extinct. Importantly, the list includes species whose extinction would likely not have been prevented by any law, including the ESA. The extinctions are concentrated in Hawaii and a swath that extends from Mississippi to Virginia. The habitats associated with extinction were tropical / subtropical forests (Hawaii) and wetlands, especially riverine systems. Invasive species were the dominant threat to extinct species, driven by the threats to Hawaiian extinct species. These results should inform how we discuss the successes of the ESA and the law's limitations.
</div>

The extinction crisis of the modern era was a key motivation for the ESA: the goals of the law include preventing extinction and recovering species so they are no longer threatened with extinction. One of the ways ESA success is measured is by comparing the number of listed species to the number that are extinct. As of this writing, ten species have been removed from the list of threatened and endangered species, and the list includes 1,652 species. Those numbers suggest `r 100 - (100 * round(10 / 1652, 3))`% of species have been saved by the ESA, which is a great accomplishment.
...
Here we identify the species that experts - biologists with the U.S. Fish and Wildlife Service (FWS) and National Marine Fisheries Service (NMFS; collectively, the Services) - have good reason to believe are or may be extinct. We mined the text of all [~1,400 five-year status reviews, totalling >30,000  pages](https://www.fws.gov/southeast/endangered-species-act/five-year-reviews/), to find a relatively [small number of phrases](#methods) that indicate extinction, then manually checked those to remove false-positives. _Declaring_ extinction may not be warranted for some of these species, but the professionals have indicated that extinction is likely to some greater or lesser degree. Getting a better handle on the number of species thought to be extinct rather than relying on the number delisted due to extinction is an important first step in thoroughly understanding the effectiveness of the ESA.  

## Possible/probable extinctions

The first challenge is to determine which species are extinct. Pinning that number down is tricky because extinction is fundamentally hard: one can't prove absence the same way we prove presence. Rather than a declaration of extinct, we set the bar at a level of experts' statements that they think the species is or may be extinct. 
...
Using [full-text searches plus manual filtering](#methods), we first identified `r length(unique(extinct$Species)) - 1` species that are believed extinct to some greater or lesser degree, plus the [snail genus _Achatinella_](#achatinella).
...
Some `r length(unique(acha$species))` species or subspecies of _Achatinella_ snails have been described, all native to the tropical forests of Oahu. Unfortunately, they have suffered more likely extinctions than any other genus of ESA-listed species. Using the status in the last five-year reviews for the genus (2011) plus information gleaned from other sources (e.g., IUCN and NatureServe), `r length(acha$believed_extinct[acha$believed_extinct == "T"])` _Achatinella_ are likely extinct:
...
In addition to the 70 species identified above, we know that some species have been delisted because they have been formally designated extinct.
...
These results indicate **`r n_ext` (`r 100*round(n_ext/(dim(tne)[1] + 41), 2)`%) current or formerly ESA-listed species are thought to be extinct.**
</div>

### Extinct in the wild

Note that these numbers do not include species that are extinct in the wild, but often number just a few in propagation or captivity. We identified `r length(ext_wild[[1]])` such species in our scans - _`r paste(ext_wild[[1]], collapse=", ")`_ - but the number is likely higher. For example, the [Plant Extinction Prevention Program](#box_1) indicated 14 Hawaiian plants that are extinct in the wild.
...
Where have extinctions occurred? Hawaii, of course, tops the list, but where are the other species?
...
Given the list of likely extinct species, we would like to know what they have in common. Any commonalities might help inform prioritization efforts that can stem future extinctions. We used data available through the [IUCN's Red List database](http://apiv3.iucnredlist.org/) for habitats and threats.
...
The [Plant Extinction Prevention (PEP) Program](http://www.pepphi.org/) works exhaustively to monitor and conserve Hawaiian plants. They also provide more up- to-date data on the status of many imperiled plants than FWS documents, and should probably be considered authoritative for PEP species.
...
Appropriately, PEP uses "?" to denote species for which extirpation and extinction are unknown. The refinement of extinct in the wild vs. extinct from the planet is also useful to consider. Last, in addition to the extinctions, the PEP data highlight the problem of extirpations: `r length(unique(extrp_wild$species))` PEP species have been (or probably have been) extirpated from one or more of the islands. First, the number of extirpations by island:
...
The PEP data adds depth to our understanding of extinctions and extirpations among Hawaiian plants, but PEP doesn't cover all ESA-listed plants.  By combining PEP table with county occurrence data, we find `r length(pep_miss)` species are not covered by PEP; more are missing than are covered by PEP. For these species in particular it is important for FWS to track species status. One way that is done is with ESA-mandated five-year reviews of species status.
